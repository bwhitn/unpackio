...
| --- | --- | --- | --- | --- |
| `aes` | 0.9.2 | MIT OR Apache-2.0 | `zeroize` | RustCrypto AES-256 primitive; `https://github.com/RustCrypto/block-ciphers` |
...
| `miniz_oxide` | 0.9.1 | MIT OR Zlib OR Apache-2.0 | `with-alloc` | Raw Deflate decoder; `https://github.com/Frommi/miniz_oxide` |
| `bzip2-rs` | 0.1.2 | MIT OR Apache-2.0 | `rustc_1_37` | Safe Rust BZip2 decoder; `https://github.com/paolobarbolini/bzip2-rs` |
| `brotli-decompressor` | 6.0.0 | BSD-3-Clause OR MIT | `std` | Brotli decoder; optional `unsafe` and `ffi-api` features disabled; `https://github.com/dropbox/rust-brotli-decompressor` |
| `lz4_flex` | 0.14.0 | MIT | `checked-decode`, `frame`, `safe-decode`, `safe-encode` | Safe checked LZ4-frame decoder for 7z coder payloads and standalone streams; the selected safe features make the crate forbid unsafe code; `https://github.com/pseitz/lz4_flex` |
| `ruzstd` | 0.8.1 | MIT | `hash`, `std` | Zstandard frame decoder with content-checksum calculation for 7z coder payloads and standalone streams; `https://github.com/KillingSpark/zstd-rs`; 0.8.2 uses APIs unavailable on Rust 1.85 and current 0.9.0 declares Rust 1.87, so both exceed this project's tested MSRV |
| `twox-hash` | 2.1.4 | MIT | `xxhash32` (plus `xxhash64` unified through `ruzstd/hash`) | LZ4 descriptor checksum validation and transitive LZ4/Zstandard frame checksum support; `https://github.com/shepmaster/twox-hash` |
| `rawzip` | 0.5.1 | MIT | none | Safe-Rust, zero-dependency ZIP/ZIP64 structural reader; writer types are neither used nor re-exported; `https://github.com/nickbabcock/rawzip` |
...
| `digest`, `block-buffer`, `hybrid-array` | 0.11.3, 0.12.1, 0.4.15 | MIT OR Apache-2.0 |
| `cpubits`, `cpufeatures`, `cfg-if`, `typenum` | 0.1.1, 0.3.1, 1.0.4, 1.20.1 | MIT OR Apache-2.0 |
| `alloc-no-stdlib`, `alloc-stdlib` | 3.0.0, 0.3.0 | BSD-3-Clause |
| `crc32fast`, `tinyvec` | 1.5.2, 1.13.3 | MIT OR Apache-2.0; Zlib OR Apache-2.0 OR MIT |
| `adler2` | 2.0.1 | MIT selected from 0BSD OR MIT OR Apache-2.0 |
| `ctutils`, `cmov` | 0.4.2, 0.5.4 | MIT OR Apache-2.0 |
| `autocfg`, `bitflags`, `chrono`, `memchr`, `num-traits` | 1.5.1, 2.13.2, 0.4.45, 2.8.3, 0.2.19 | Apache-2.0 OR MIT; MIT OR Apache-2.0; MIT OR Apache-2.0; MIT selected from Unlicense OR MIT; MIT OR Apache-2.0 |
...
Zstandard adapters. The ARJ reader adds exact `delharc` 0.6.2 only for methods
...
writer modules are not imported or re-exported. Method 95 reuses the shared
checked XZ Stream/Block/Index adapter and delegates each already bounded
Compressed Data field to the existing checked, fallibly allocating in-tree
LZMA2 decoder; it does not call `lzma-rust2`'s `Lzma2Reader` or optional
aggregate `XzReader`, add another ZIP parser, or invoke an external command.
Method 98 adds no runtime dependency: its checked
PPMd-I revision-1 range decoder, model, and suballocator are an in-tree safe
...
construction because it is not cryptographically secure and has no admitted
runtime implementation dependency.

The RPM layer reuses already admitted BZip2, Deflate, and Zstandard decoders.
`lzma-rust2` supplies legacy LZMA-alone entropy decoding in safe Rust; its
encoder/optimization defaults and optional XZ container are disabled. XZ
LZMA2 Blocks use the in-tree checked BSD-3-Clause/MIT adaptation instead of the
dependency's infallibly allocated dictionary buffer. The shared project wrapper parses XZ headers, Index records, Blocks, filter
properties, dictionaries, padding, and Checks before or around each bounded
Block decode, charges the declared dictionary, bounds output, and checkpoints
controlled input/output/checksum loops. RPM header, digest, and CPIO parsing
add no dependency.
...
| --- | --- | --- | --- | --- |
| `pyo3` | 0.29.2 | MIT OR Apache-2.0 | `abi3-py39`, `macros`; `extension-module` only for wheels | CPython ABI/type/call adapter; `https://github.com/PyO3/pyo3` |
...
| `unpackio` | 0.2.0, local path | MIT plus recorded adapted-source notices | normal core features only | Sole parser/model/decoder/crypto implementation |

PyO3 resolves `pyo3-build-config`, `pyo3-ffi`, `pyo3-macros`, and
`pyo3-macros-backend` 0.29.2; `libc` 0.2.189; `once_cell` 1.21.4;
`portable-atomic` 1.15.0; `proc-macro2` 1.0.107; `quote` 1.0.47; `syn`
...
resolution selects the same exact `twox-hash` 2.1.4 (MIT) as the root
...
Maturin 1.15.0 is pinned as the PEP 517 build backend and CI packaging tool; it
...
`LICENSES/`, and `NOTICE`. The sdist configuration excludes `dist*/**`, and CI
rejects nested wheels or distribution-output directories before rebuilding the
archive. The CI-only `PyO3/maturin-action` is pinned to commit
`e83996d129638aa358a18fbd1dfb82f0b0fb5d3b` (v1.51.0, MIT).
...
`dc37677b2e1c63e2034f94d8a5b11f265b73ba33` (v1.14.2, BSD-3-Clause) and runs
...
release path pins `actions/upload-artifact` v7.0.1 at
`043fb46d1a93c77aae656e7c1c64a875d1fc6a0a` and
`actions/download-artifact` v8.0.1 at
`3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`; both use MIT. Checkout and
Python setup are also pinned to `actions/checkout` v7.0.1 at
`3d3c42e5aac5ba805825da76410c181273ba90b1` and `actions/setup-python`
v7.0.0 at `5fda3b95a4ea91299a34e894583c3862153e4b97`; their Node 24 action
runtimes are supported by the selected GitHub-hosted runners. Those actions,
GitHub-hosted runners, and PyPI are
...
The fuzz package directly names `aes` 0.9.2 and `cbc` 0.2.1 only to author a
...
The 2026-09-14 version-0.2.0 dependency refresh compared every direct crate
with the current crates.io sparse index and every packaging/action dependency
with its authoritative PyPI or GitHub release. It updated `aes` 0.9.1 to
0.9.2, `brotli-decompressor` 5.0.3 to 6.0.0, `delharc` 0.6.1 to 0.6.2,
`lz4_flex` 0.13.1 to 0.14.0, `lzma-rust2` 0.16.4 to 0.20.1,
`miniz_oxide` 0.8.9 to 0.9.1, `rawzip` 0.4.4 to 0.5.1, `twox-hash` 2.1.2
to 2.1.4, and PyO3 0.29.0 to 0.29.2. The exact feature surface remains
decoder-only and safe-code where available; no encoder, writer, native FFI,
or runtime fallback was enabled. The refresh also selected current compatible
transitives in all three lockfiles and passed cargo-deny 0.20.2 for
advisories, bans, licenses, and sources in the core, binding, and fuzz graphs.
`aes` 0.9.3 is intentionally excluded because it declares Rust 1.89;
`delharc` 0.7.0 and 0.8.0 declare Rust 1.93 and 1.95; `ruzstd` 0.8.2 uses
integer APIs unavailable on Rust 1.85 and 0.9.0 declares Rust 1.87. The
selected graph was compiled and tested with the official Rust 1.85.0 image.

The 2026-09-14 ZIP advanced-method review also changed no resolved package.
The following exact candidates were inspected and rejected before admission:

- `ppmd-rust` 1.4.0/1.5.0 is derived from 7-Zip, uses extensive unsafe pointer
...
- official WavPack 5.9.0 at commit
  ...
  parser rather than the project's checked Rust model. `wavpack-rs` at commit
  `009766e5d06a6072f3b95a268d73089abd2726ed` declares MIT in metadata but has
  no license file at that revision, disables overflow checks, performs
  unchecked input indexing/casts/allocation and unbounded unary decoding, and
  implements only part of the PCM surface without exact RIFF wrapper recovery.
- libarchive 3.8.9 at commit
  `27cbc7827172698143e440801fc0ba39ccb4f1f5` is BSD-licensed native C. It names
  the ZIP method identifiers but supplies no WinZip JPEG or WavPack decoder,
...
| AES-256 | RustCrypto `aes` family | `aes` 0.9.2 admitted with `zeroize` |
...
| Deflate | safe permissive implementation | `miniz_oxide` 0.9.1 admitted for Deflate |
...
| Python FFI | isolated adapter over the stable core | PyO3 0.29.2 admitted in `bindings/python`; no Python dependency enters the core workspace |
